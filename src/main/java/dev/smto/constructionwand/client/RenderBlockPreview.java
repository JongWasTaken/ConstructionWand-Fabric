...
public class RenderBlockPreview {
    ...
    public static boolean renderBlockHighlight(LevelRenderContext context, BlockOutlineRenderState outlineRenderState) {
        ...
        if (!(player.isShiftKeyDown() && ClientEvents.isOptKeyDown())) {
            ...
            if (
                    RenderBlockPreview.wandJob == null ||
                            !RenderBlockPreview.compareRTR(RenderBlockPreview.wandJob.rayTraceResult, rtr) ||
                            !(RenderBlockPreview.wandJob.wand.equals(wand)) ||
                            RenderBlockPreview.wandJob.blockCount() < 2
            ) {
                RenderBlockPreview.wandJob = new WandJob(player, player.level(), rtr, wand);
            }
            blocks = RenderBlockPreview.wandJob.getBlockPositions();
        } else {
            blocks = RenderBlockPreview.undoBlocks;
            colorG = 255;
        }
